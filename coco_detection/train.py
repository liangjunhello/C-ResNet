...
from faster_rcnn import AnchorsGenerator,FasterRCNN
from group_by_aspect_ratio import create_aspect_ratio_groups, GroupedBatchSampler
from torchvision.transforms import InterpolationMode
from transforms import SimpleCopyPaste


def create_model(num_classes,blockid,layers):
    import torchvision
    from torchvision.models.feature_extraction import create_feature_extractor
    from resnetimprove import net_construct

    backbone = net_construct(blockid,layers)
    print(backbone)
    backbone = create_feature_extractor(backbone, return_nodes={"layer4": "0"})
    #
    backbone.out_channels = backbone.layer4[0].conv6.out_channels
...
def get_args_parser(add_help=True):
    ...
    # model argument
    parser.add_argument("--blockid", default=9, type=int,
    ...
    parser.add_argument('--layers', default='1,1,1,1', type=str,
                        help='num of layer,like resnet18:2,2,2,2 resnet34:3,4,6,3 c-resnet-B2:1,1,1,1')
    return parser
...
#     )
    ...
    if args.weights_backbone:
        weights_dict = torch.load(args.weights_backbone,map_location="cpu")
        weights_dict = cleanup_state_dict(weights_dict)
        for k in list(weights_dict.keys()):
            if "linear" in k:
                del weights_dict[k]
        model_without_ddp.backbone.load_state_dict(weights_dict,strict=True)
    if args.resume:
